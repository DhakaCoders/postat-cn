		...
		echo '<select name="' . $field['name'] . '" class="acf-menu-chooser">';
				...
				if ( ! empty( $menus ) ) {
					foreach ( $menus as $choice ) {
						$field['choices'][ $choice->slug ] = $choice->slug;
						$field['choices'][ $choice->name ] = $choice->name;

						echo '<option  value="' . $field['choices'][ $choice->slug ] . '" ' . selected($field_value, $field['choices'][ $choice->slug ], false) . ' >' . $field['choices'][ $choice->name ] . '</option>' ;
					}
				}
		echo '</select>';
